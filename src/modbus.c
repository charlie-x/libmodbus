...
void _error_print ( modbus_t *ctx, const char *context )
{
    if ( ctx->debug ) {
        int err;
        _get_errno ( &err );
        _RPT1 ( _CRT_WARN, "ERROR %s", modbus_strerror ( err ) );
...
static void _sleep_response_timeout ( modbus_t *ctx )
{
    ...
    while ( nanosleep ( &request, &remaining ) == -1 && _set_errno ( = EINTR ) {
    request = remaining;
}
...
int modbus_flush ( modbus_t *ctx )
{
    ...
    if ( rc != -1 && ctx->debug ) {
        ...
        _RPT1 ( _CRT_WARN, "Bytes flushed (%d)\n", rc );
    }
    ...
    do {
        ...
        if ( rc == -1 ) {
            ...
            if ( ctx->error_recovery & MODBUS_ERROR_RECOVERY_LINK ) {
                ...
                if ( ( errno == EBADF || saved_errno == ECONNRESET || saved_errno == EPIPE ) ) {
                    modbus_close ( ctx );
    ...
    if ( msg_type == MSG_INDICATION ) {
        if ( function <= MODBUS_FC_WRITE_SINGLE_REGISTER ) {
            ...
            if ( function == MODBUS_FC_WRITE_MULTIPLE_COILS ||
                ...
                if ( function == MODBUS_FC_MASK_WRITE_REGISTER ) {
                    length = 6;
...
int _modbus_receive_msg ( modbus_t *ctx, uint8_t *msg, msg_type_t msg_type )
{
    ...
    if ( ctx->debug ) {
        if ( msg_type == MSG_INDICATION ) {
            _RPT0 ( _CRT_WARN, "Waiting for a indication...\n" );

        } else {
            _RPT0 ( _CRT_WARN, "Waiting for a confirmation...\n" );
        }
    }
    ...
    while ( length_to_read != 0 ) {
        ...
        if ( ctx->debug ) {
            ...
            for ( i = 0; i < rc; i++ )
            { _RPT1 ( _CRT_WARN, "<%.2X>", msg[msg_length + i] ); }
        }
    ...
    if ( function >= 0x80 ) {
        if ( rsp_length == ( offset + 2 + ( int ) ctx->backend->checksum_length ) &&
            ...
            if ( exception_code < MODBUS_EXCEPTION_MAX ) {
                _set_errno ( MODBUS_ENOBASE + exception_code );

            } else {
                _set_errno ( EMBBADEXC );
            }
    ...
    switch ( function ) {
        case MODBUS_FC_READ_COILS: {
                ...
                if ( nb < 1 || MODBUS_MAX_READ_BITS < nb ) {
                    ...
                    if ( address < mb_mapping->offset_bits || ( addr + nb ) > mb_mapping->nb_bits ) {
                        if ( ctx->debug ) {
                            _RPT1 ( _CRT_WARN, "Illegal data address 0x%0X in read_bits\n",
        ...
        case MODBUS_FC_READ_DISCRETE_INPUTS: {
                ...
                if ( nb < 1 || MODBUS_MAX_READ_BITS < nb ) {
                    ...
                    if ( address < mb_mapping->offset_input_bits || ( addr + nb ) > mb_mapping->nb_input_bits ) {
                        if ( ctx->debug ) {
                            _RPT1 ( _CRT_WARN, "Illegal data address 0x%0X in read_input_bits\n",
        ...
        case MODBUS_FC_READ_HOLDING_REGISTERS: {
                ...
                if ( nb < 1 || MODBUS_MAX_READ_REGISTERS < nb ) {
                    ...
                    if ( address < mb_mapping->offset_registers || ( addr + nb ) > mb_mapping->nb_registers ) {
                        if ( ctx->debug ) {
                            _RPT1 ( _CRT_WARN, "Illegal data address 0x%0X in read_registers\n",
        ...
        case MODBUS_FC_READ_INPUT_REGISTERS: {
                ...
                if ( nb < 1 || MODBUS_MAX_READ_REGISTERS < nb ) {
                    ...
                    if ( address < mb_mapping->offset_input_registers || ( addr + nb ) > mb_mapping->nb_input_registers ) {
                        if ( ctx->debug ) {
                            _RPT1 ( _CRT_WARN, "Illegal data address 0x%0X in read_input_registers\n",
        ...
        case MODBUS_FC_WRITE_SINGLE_REGISTER: {
                ...
                if ( address < mb_mapping->offset_registers || addr >= mb_mapping->nb_registers ) {
                    if ( ctx->debug ) {
                        _RPT1 ( _CRT_WARN, "Illegal data address 0x%0X in write_register\n",
                                address );
                    }
        ...
        case MODBUS_FC_WRITE_MULTIPLE_COILS: {
                ...
                if ( nb < 1 || MODBUS_MAX_WRITE_BITS < nb ) {
                    ...
                    if ( address < mb_mapping->offset_bits || ( addr + nb ) > mb_mapping->nb_bits ) {
                        if ( ctx->debug ) {
                            _RPT1 ( _CRT_WARN, "Illegal data address 0x%0X in write_bits\n",
                                    address < mb_mapping->offset_bits ? address : address + nb );
                        }
        ...
        case MODBUS_FC_WRITE_MULTIPLE_REGISTERS: {
                ...
                if ( nb < 1 || MODBUS_MAX_WRITE_REGISTERS < nb ) {
                    ...
                    if ( address < mb_mapping->offset_registers || ( addr + nb ) > mb_mapping->nb_registers ) {
                        if ( ctx->debug ) {
                            _RPT1 ( _CRT_WARN, "Illegal data address 0x%0X in write_registers\n",
                                    address < mb_mapping->offset_registers ? address : address + nb );
                        }
        ...
        case MODBUS_FC_READ_EXCEPTION_STATUS:
            if ( ctx->debug ) {
                _RPT0 ( _CRT_WARN, "FIXME Not implemented\n" );
            }

            _set_errno (  ENOPROTOOPT );
            return -1;
            break;
...
int modbus_write_bits ( modbus_t *ctx, int addr, int nb, const uint8_t *src )
{
    ...
    if ( nb > MODBUS_MAX_WRITE_BITS ) {
        if ( ctx->debug ) {
            _RPT1 ( _CRT_WARN, "ERROR Writing too many bits (%d > %d)\n",
                    nb, MODBUS_MAX_WRITE_BITS );
        }
